...
$X$を楕円曲線としたとき, $X$の点の形式的有限和$D = \sum a_n(P_n)$をWeil因子といい, その次数を$\deg D = \sum a_n$で定義します. $X$の因子全体の集合を$\Div(X)$と書きます. 因子の和を$D + D' = \sum a_n(P_n) + \sum a'_n (P'_n)$とすることで$\Div(X)$はAbel群になります. $\Div^0(X) = \Set{D \in \Div(X) | \deg D = 0}$とすると$\Div^0(X)$は$\Div(X)$の部分群です. 

順序も入れておきましょう. 2つの因子$D, D'$について$D \geq D'$であるとは, $D - D' = \sum a_n(P_n)$について$a_n \geq 0 \mathrm{\ for\ all\ } n$とします.
...
\underline{楕円曲線に対する一番簡単なRiemann-Rochの定理(の一部)}: $\deg D > 0$のとき$l(D) = \deg D$.\\
